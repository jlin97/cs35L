...
#define camera_fov 75.0*(PI/180.0)

// scene that will be rendered
scene_t scene;

// contains starting and ending values
struct ScenePackages{
    int start;
    int end;
};

// contains values for each pixel
float image_values[width][height][3];

void *divided_load(void * current_item){
    Vec3 camera_pos;
    set( camera_pos, 0., 0., -4. );
    Vec3 bg_color;
    set( bg_color, 0.8, 0.8, 1 );

    // stores the passed argument into a variable
    struct ScenePackages context = *(struct ScenePackages *)current_item;
    ...
    // start and end dictate how the rows get allocated to each thread
    for( int px=context.start; px<context.end; px++ )
    { 
        ...
        for( int py=0; py<height; ++py )
        {
             ...
             */
            image_values[px][py][0] = scaled_color[0];
            image_values[px][py][1] = scaled_color[1];
            image_values[px][py][2] = scaled_color[2];
        }
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    // lesson: an array of structs has to be used instead of a single struct
    // this is to avoid race conditions with multiple threads. 
    struct ScenePackages current[nthreads];    
    
    int ret = 0;
    // splitting up the width
    int width_for_thread = width/nthreads;

    // delegating work to nthreads-1 threads; last thread saved for remainder rows
    for(int i = 0; i < nthreads-1; i++ ){
        current[i].start = width_for_thread*i;
        current[i].end = width_for_thread*(i+1);
        ret = pthread_create( &threads[i], NULL, divided_load, &current[i] );
        if(ret){
    ...
    if(nthreads != 1){
    	// delegating work to the final thread which will work on the (width%nthreads) rows
    	...
    	divided_load( &current[nthreads-1]);
    }
    // printing out all the values after each pixel's value has been determined
    for(int x = 0; x < width; x++){
    	for(int y = 0; y < height; y++){
            printf("%.0f %.0f %.0f\n", image_values[x][y][0], image_values[x][y][1], image_values[x][y][2]);
        }
        printf("\n");
    } 
